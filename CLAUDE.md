...
- **Íconos de marca**: `src/components/icons/nora/` — set propio de SVGs (máscaras, claqueta, cortina, spotlight, ticket, firma, estrella). Todo lo que exporta `index.ts` está en uso real; si se agrega un ícono nuevo al set y no se termina usando, sacarlo del barrel y borrar el archivo en la misma sesión (no dejarlo "por si se usa después" — ver la nota de limpieza más abajo).

**Componentes que existieron y se borraron por no tener ningún uso** (2026-09-12): `ProgramIndex.tsx`, `Highlights.tsx`, `SiteBanner.tsx`, `IntroCinematic.tsx`, `PillarMenu.tsx`, y 5 íconos de `icons/nora/` que nunca llegaron a importarse en ningún lado. Están en el historial de git si hace falta recuperar alguno — no reflotarlos por comodidad sin confirmar que el enfoque actual (`SectionNav` para nav, `VerticalPhotoSlider` para archivo) no ya lo resuelve.
